...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const { slug } = await params; const post = await resolvePost(slug); if (!post) return { title: 'Not Found' }; return { title: post.title }; }
...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <div>
      ...
      <article className="py-12 bg-white"><div className="max-w-4xl mx-auto px-4"><div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: contentToHtml(post.content) }} /><Link href="/blog" className="inline-block mt-8 text-purple-600 font-medium">← Back to Blog</Link></div></article>
    </div>
  );
}
