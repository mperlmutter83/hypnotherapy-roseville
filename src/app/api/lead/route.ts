...
const FROM = "Hypnotherapy Roseville <info@yesidoinc.com>";
...
async function notifyWorkApplicant(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;color:#111;">
      <h2 style="margin:0 0 12px;">New work applicant — hypnotherapyroseville.com</h2>
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New work applicant — ${opts.name} (hypnotherapyroseville.com)`,
        html,
      }),
    });
    if (!res.ok) {
  ...
  try {
    if (!CRM_FORM_KEY) {
      ...
      return NextResponse.json(
        { error: "Lead form is not configured" },
        { status: 500 }
      );
    }
    ...
    const serviceNeeded = [
      `Service: ${service || "Hypnotherapy"}`,
      message ? `Details: ${message}` : null,
      "Source: hypnotherapyroseville.com",
    ]
      .filter(Boolean)
      .join(" | ");

    // All leads are forwarded to the Yes Crew CRM.
